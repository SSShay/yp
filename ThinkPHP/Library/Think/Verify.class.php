...
class Verify {
    protected $config =	array(
        ...
        'codeSet'   =>  '234567abcdefhijkmnpqrstuvwxyzACDEFGHJKLMNPQRTUVWXY',             // 验证码字符集合
    ...
    public function check($code, $id = '')
    {
        ...
        if (strtoupper($code) == $secode['code']) {
            $this->reset && session('verify' . $id, null);
            return true;
        }
        $this->error = '验证码错误！';
        return false;
    ...
    public function entry($id = '')
    {
        ...
        // 图片高(px)
        $this->imageH || $this->imageH = $this->fontSize * 1.875;
        ...
        if ($this->useZh) { // 中文验证码
            ...
            for ($i = 0; $i < $this->length; $i++) {
                ...
                $codeNX += mt_rand($this->fontSize * 1.325, $this->fontSize * 1.5);
                imagettftext($this->_image, $this->fontSize, mt_rand(-40, 40), $codeNX, $this->fontSize * 1.325, $this->_color, $this->fontttf, $code[$i]);
            }
        }

        // 保存验证码
        $secode['code'] = strtoupper(implode('', $code)); // 把校验码保存到session
        $secode['time'] = NOW_TIME;  // 验证码创建时间
        session('verify'.$id, $secode);
